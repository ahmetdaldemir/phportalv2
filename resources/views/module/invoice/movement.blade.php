                                        ...
                                        <select name="invoice_id" class="form-control">
                                            @foreach($invoices as $invoice)
                                                <option @if(isset($invoices))
                                                            {{ $invoices->hasSeller($invoice->id) ? 'selected' : '' }}
                                                        @endif  value="{{$invoice->id}}">{{$invoice->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="mb-3 col-lg-4 col-xl-2 col-12 mb-0">
                                        ...
                                        <select name="warehouse_id" class="form-control">
                                            @foreach($warehouses as $warehouse)
                                                <option @if(isset($invoices))
                                                            {{ $invoices->hasWarehouse($warehouse->id) ? 'selected' : '' }}
                                    ...
                                    <div class="mb-3 col-lg-4 col-xl-2 col-12 mb-0">
                                        ...
                                        <select name="color_id" class="form-control">
                                            @foreach($colors as $color)
                                                <option @if(isset($invoices))
                                                            {{ $invoices->hasColor($color->id) ? 'selected' : '' }}
                ...
                <table class="table">
                    ...
                    @foreach($movements as $movement)
                        <tr>
                            ...
                            <td><strong>{{$movement->invoice->name}}</strong></td>
                            <td><strong>{{$movement->warehouse->name}}</strong></td>
                            <td>
                                <a href="{{route('invoice.showmovemnet',['id' => 1])}}"
                                   class="btn btn-icon btn-success">
                                    <span class="bx bxl-edge"></span>
                                </a>
                                <a href="{{route('invoice.edit',['id' => 1])}}"
                                   class="btn btn-icon btn-primary">
                                    <span class="bx bx-edit-alt"></span>
                                </a>
                                <a href="{{route('invoice.delete',['id' => 1])}}"
