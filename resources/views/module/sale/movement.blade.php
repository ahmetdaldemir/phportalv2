    ...
    <div class="container-xxl flex-grow-1 container-p-y">
                                    ...
                                    <div class="mb-3 col-lg-4 col-xl-2 col-12 mb-0">
                                        ...
                                        <select name="sale_id" class="form-control">
                                            @foreach($sales as $sale)
                                                <option @if(isset($sales))
                                                            {{ $sales->hasSeller($sale->id) ? 'selected' : '' }}
                                                        @endif  value="{{$sale->id}}">{{$sale->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="mb-3 col-lg-4 col-xl-2 col-12 mb-0">
                                        ...
                                        <select name="warehouse_id" class="form-control">
                                            @foreach($warehouses as $warehouse)
                                                <option @if(isset($sales))
                                                            {{ $sales->hasWarehouse($warehouse->id) ? 'selected' : '' }}
                                    ...
                                    <div class="mb-3 col-lg-4 col-xl-2 col-12 mb-0">
                                        ...
                                        <select name="color_id" class="form-control">
                                            @foreach($colors as $color)
                                                <option @if(isset($sales))
                                                            {{ $sales->hasColor($color->id) ? 'selected' : '' }}
                                    ...
                                    <div class="mb-3 col-lg-4 col-xl-2 col-12 mb-0">
                                        ...
                                        <select name="reason_id" class="form-control">
                                            @foreach($reasons as $reason)
                                                <option @if(isset($sales))
                                                            {{ $sales->hasReason($reason->id) ? 'selected' : '' }}
        ...
        <div class="card">
            <div class="card-header">
                <a href="{{route('sale.create')}}" class="btn btn-primary float-end">Yeni Stok Kartı Ekle</a>
            </div>
                ...
                <table class="table">
                    ...
                    @foreach($movements as $movement)
                        <tr>
                            ...
                            <td><strong>{{$movement->sale->name}}</strong></td>
                            <td><strong>{{$movement->warehouse->name}}</strong></td>
                            <td>
                                <a href="{{route('sale.showmovemnet',['id' => 1])}}"
                                   class="btn btn-icon btn-success">
                                    <span class="bx bxl-edge"></span>
                                </a>
                                <a href="{{route('sale.edit',['id' => 1])}}"
                                   class="btn btn-icon btn-primary">
                                    <span class="bx bx-edit-alt"></span>
                                </a>
                                <a href="{{route('sale.delete',['id' => 1])}}"
